inherited FrmEntradaVisitante: TFrmEntradaVisitante
  ClientHeight = 601
  ClientWidth = 989
  Caption = 'Confere'
  OnShow = nil
  ExplicitWidth = 1005
  ExplicitHeight = 640
  PixelsPerInch = 96
  TextHeight = 13
  inherited PanelTituloModeloCadastro: TUniPanel
    Width = 989
    Height = 52
    ExplicitWidth = 989
    ExplicitHeight = 52
    inherited UniImageLogoMarca: TUniImage
      AlignWithMargins = True
      Left = 3
      Top = 3
      Width = 58
      Height = 46
      Picture.Data = {
        
        4CC6C946DBF51D8E0C18301FFFD9}
      Transparent = False
      ExplicitLeft = 3
      ExplicitTop = 3
      ExplicitWidth = 58
      ExplicitHeight = 46
    end
    inherited PanelLocalizaConsulta: TUniPanel
      Left = 64
      Width = 925
      Height = 50
      TabOrder = 1
      ExplicitLeft = 64
      ExplicitWidth = 925
      ExplicitHeight = 50
      inherited UniLabel2: TUniLabel
        Left = 128
        Top = 20
        ExplicitLeft = 128
        ExplicitTop = 20
      end
      inherited EditLocalizar: TUniEdit
        Left = 179
        Top = 14
        ScreenMask.Enabled = True
        ScreenMask.Message = 'Pesquisando registros... aguarde...'
        ScreenMask.Target = DBGridConsulta
        CheckChangeDelay = 50
        OnChange = nil
        ExplicitLeft = 179
        ExplicitTop = 14
      end
      inherited UniBtnFiltrar: TUniBitBtn
        Left = 753
        Top = 20
        ExplicitLeft = 753
        ExplicitTop = 20
      end
      object UniRadioGroupStatus: TUniRadioGroup
        Left = 16
        Top = 1
        Width = 97
        Height = 45
        Items.Strings = (
          'Ativo'
          'Inativo')
        ItemIndex = 0
        TabOrder = 4
      end
    end
  end
  inherited PanelBotoes: TUniPanel
    Top = 52
    Width = 989
    ExplicitTop = 52
    ExplicitWidth = 989
    inherited ToolBarModeloCadastro: TUniToolBar
      Width = 983
      ExplicitWidth = 983
      inherited Novo: TUniToolButton
        ExplicitHeight = 43
      end
      inherited Editar: TUniToolButton
        ExplicitHeight = 43
      end
      inherited UniToolButton8: TUniToolButton
        ExplicitHeight = 43
      end
      inherited Cancelar: TUniToolButton
        ExplicitHeight = 43
      end
      inherited Salvar: TUniToolButton
        ExplicitHeight = 43
      end
      inherited UniToolButton7: TUniToolButton
        ExplicitHeight = 43
      end
      inherited Excluir: TUniToolButton
        ExplicitHeight = 43
      end
      inherited Fechar: TUniToolButton
        ExplicitHeight = 43
      end
    end
  end
  inherited PanelModeloCadastro: TUniPanel
    Top = 107
    Width = 989
    Height = 468
    ExplicitTop = 107
    ExplicitWidth = 1061
    ExplicitHeight = 463
    inherited PageControlModeloCadastro: TUniPageControl
      Width = 987
      Height = 177
      ExplicitWidth = 987
      ExplicitHeight = 177
      inherited TabSheetConsulta: TUniTabSheet
        ExplicitLeft = 4
        ExplicitTop = 24
        ExplicitWidth = 979
        ExplicitHeight = 149
        inherited DBGridConsulta: TUniDBGrid
          Width = 979
          Height = 149
          DataSource = DsConsulta
          Columns = <
            item
              FieldName = 'NOME_INTERNO'
              Title.Caption = 'Nome do Interno'
              Width = 397
              Visible = True
              Font.Height = -13
              Font.Name = 'Lucida Console'
              Font.Style = [fsBold]
              Expanded = False
            end
            item
              FieldName = 'RGI'
              Title.Caption = 'Prontu'#225'rio'
              Width = 60
              Visible = True
              Expanded = False
            end
            item
              FieldName = 'NUMERO_ROUPA'
              Title.Caption = 'Roupa'
              Width = 40
              Visible = True
              Expanded = False
            end
            item
              FieldName = 'SIGLA'
              Title.Caption = 'Sigla'
              Width = 60
              Visible = True
              Expanded = False
            end
            item
              FieldName = 'PAVILHAO'
              Title.Caption = 'Pavilh'#227'o'
              Width = 95
              Visible = True
              Expanded = False
            end
            item
              FieldName = 'GALERIA'
              Title.Caption = 'Galeria'
              Width = 60
              Visible = True
              Expanded = False
            end
            item
              FieldName = 'SOLARIO'
              Title.Caption = 'Solario'
              Width = 56
              Visible = True
              Expanded = False
            end
            item
              FieldName = 'CELA'
              Title.Caption = 'Cela'
              Width = 40
              Visible = True
              Expanded = False
            end
            item
              FieldName = 'STATUS'
              Title.Caption = 'Status'
              Width = 56
              Visible = True
              Expanded = False
            end
            item
              FieldName = 'EM_TRANSITO'
              Title.Caption = 'Transito'
              Width = 43
              Visible = True
              Expanded = False
            end
            item
              FieldName = 'id_me'
              Title.Caption = 'Monitorado'
              Width = 63
              Visible = True
              Expanded = False
            end>
        end
      end
    end
    inherited PanelCadastro: TUniPanel
      Top = 177
      Width = 987
      Height = 291
      ExplicitTop = 177
      ExplicitWidth = 987
      ExplicitHeight = 291
      inherited PageControlCadastro: TUniPageControl
        Width = 985
        Height = 289
        ExplicitWidth = 977
        ExplicitHeight = 267
        inherited UniTabSheetPrincipalCadastro: TUniTabSheet
          ExplicitLeft = 4
          ExplicitTop = 24
          ExplicitWidth = 977
          ExplicitHeight = 261
          object UniPageControlConfere: TUniPageControl
            Left = 0
            Top = 56
            Width = 977
            Height = 205
            ActivePage = UniTabSheetDados
            TabOrder = 10
            Align = alBottom
            Anchors = [akLeft, akRight, akBottom]
            ExplicitTop = 55
            ExplicitWidth = 969
            object UniTabSheetDados: TUniTabSheet
              Caption = 'Dados'
              object UniLabel6: TUniLabel
                Left = 3
                Top = 1
                Width = 79
                Height = 13
                Caption = 'RGI (Prontu'#225'rio)'
                TabOrder = 0
              end
              object UniDBEditRGI: TUniDBEdit
                Left = 3
                Top = 20
                Width = 83
                Height = 22
                DataField = 'RGI'
                DataSource = DsCadastro
                TabOrder = 3
                Color = clWindow
              end
              object UniLabel7: TUniLabel
                Left = 94
                Top = 3
                Width = 58
                Height = 13
                Caption = 'Proced'#234'ncia'
                TabOrder = 1
              end
              object DBLookupComboBoxID_PROCEDENCIA: TUniDBLookupComboBox
                Left = 94
                Top = 21
                Width = 404
                ListField = 'PROCEDENCIA'
                ListSource = Dm.DsProcedencia
                KeyField = 'ID_PROCEDENCIA'
                ListFieldIndex = 0
                DataField = 'ID_PROCEDENCIA'
                DataSource = DsCadastro
                TabOrder = 4
              end
              object UniDBComboBoxSexo: TUniDBComboBox
                Left = 507
                Top = 21
                Width = 60
                DataField = 'SEXO'
                DataSource = DsCadastro
                Items.Strings = (
                  'M'
                  'F')
                ItemIndex = -1
                TabOrder = 5
              end
              object UniLabel8: TUniLabel
                Left = 507
                Top = 3
                Width = 24
                Height = 13
                Caption = 'Sexo'
                TabOrder = 2
              end
              object UniLabel9: TUniLabel
                Left = 3
                Top = 45
                Width = 20
                Height = 13
                Caption = 'M'#227'e'
                TabOrder = 6
              end
              object UniDBEditMae: TUniDBEdit
                Left = 3
                Top = 62
                Width = 280
                Height = 22
                DataField = 'MAE'
                DataSource = DsCadastro
                TabOrder = 8
                Color = clWindow
              end
              object UniLabel10: TUniLabel
                Left = 287
                Top = 45
                Width = 14
                Height = 13
                Caption = 'Pai'
                TabOrder = 7
              end
              object UniDBEdit4: TUniDBEdit
                Left = 287
                Top = 62
                Width = 280
                Height = 22
                DataField = 'PAI'
                DataSource = DsCadastro
                TabOrder = 9
                Color = clWindow
              end
              object UniLabel11: TUniLabel
                Left = 3
                Top = 90
                Width = 77
                Height = 13
                Caption = 'Regime Prisional'
                TabOrder = 10
              end
              object UniDBLookupComboBox2: TUniDBLookupComboBox
                Left = 289
                Top = 107
                Width = 280
                ListField = 'DESCRICAO'
                ListSource = Dm.DsCondicaoInterno
                KeyField = 'IDCONDICAO_INTERNO'
                ListFieldIndex = 0
                DataField = 'IDCONDICAO_INTERNO'
                DataSource = DsCadastro
                TabOrder = 13
              end
              object UniLabel12: TUniLabel
                Left = 287
                Top = 90
                Width = 98
                Height = 13
                Caption = 'Condi'#231#227'o do Interno'
                TabOrder = 11
              end
              object UniDBComboBox2: TUniDBComboBox
                Left = 3
                Top = 107
                Width = 280
                DataField = 'REGIME_ATUAL'
                DataSource = DsCadastro
                Items.Strings = (
                  'Aberto'
                  'Fechado'
                  'Mandado de Pris'#227'o C'#237'vel'
                  'Medida de Seguran'#231'a'
                  'Medida de Tratamento'
                  'Provis'#243'rio'
                  'Semi-Aberto Sem Sa'#237'da'
                  'Semi-Aberto Com Trabalho Externo'
                  'Semi-Aberto Com Sa'#237'da'
                  'Semi-Aberto Com Trabalho Concedido'
                  'Semi-Aberto Com Trabalho Concedido e Sa'#237'da'
                  'Semi-Aberto Com Trabalho e Sa'#237'da'
                  'Tempor'#225'rio')
                ItemIndex = -1
                TabOrder = 12
              end
              object UniLabel13: TUniLabel
                Left = 3
                Top = 132
                Width = 26
                Height = 13
                Caption = 'Vulgo'
                TabOrder = 14
              end
              object UniDBEdit5: TUniDBEdit
                Left = 3
                Top = 149
                Width = 280
                Height = 22
                DataField = 'VULGO'
                DataSource = DsCadastro
                TabOrder = 16
                Color = clWindow
              end
              object UniDBComboBox3: TUniDBComboBox
                Left = 289
                Top = 149
                Width = 114
                DataField = 'DADOS_OUTRO_BANCO'
                DataSource = DsCadastro
                Items.Strings = (
                  'SIM'
                  'N'#195'O')
                ItemIndex = -1
                TabOrder = 17
              end
              object UniLabel14: TUniLabel
                Left = 289
                Top = 135
                Width = 114
                Height = 13
                Caption = 'Informa'#231#245'es Externas? '
                TabOrder = 15
              end
              object DBEditID: TUniDBEdit
                Left = 486
                Top = 151
                Width = 83
                Height = 22
                Enabled = False
                DataField = 'ID_INTERNO'
                DataSource = DsCadastro
                TabOrder = 18
                Color = clWindow
              end
              object UniLabel15: TUniLabel
                Left = 486
                Top = 135
                Width = 11
                Height = 13
                Caption = 'ID'
                TabOrder = 19
              end
              object UniDBImage1: TUniDBImage
                Left = 580
                Top = 21
                Width = 333
                Height = 162
                DataField = 'FOTO'
                DataSource = DsCadastro
                Center = True
                Stretch = True
                Proportional = True
              end
            end
          end
          object UniDBEditNome: TUniDBEdit
            Left = 11
            Top = 28
            Width = 382
            Height = 22
            DataField = 'NOME_INTERNO'
            DataSource = DsCadastro
            TabOrder = 4
            Color = clWindow
          end
          object UniLabel1: TUniLabel
            Left = 11
            Top = 9
            Width = 27
            Height = 13
            Caption = 'Nome'
            TabOrder = 0
          end
          object UniDBEditNUMERO_ROUPA: TUniDBEdit
            Left = 398
            Top = 28
            Width = 83
            Height = 22
            DataField = 'NUMERO_ROUPA'
            DataSource = DsCadastro
            TabOrder = 5
            Color = clWindow
          end
          object UniLabel3: TUniLabel
            Left = 398
            Top = 9
            Width = 71
            Height = 13
            Caption = 'Numero Roupa'
            TabOrder = 1
          end
          object UniLabel4: TUniLabel
            Left = 485
            Top = 9
            Width = 97
            Height = 13
            Caption = 'CI/OF. MOVIMENTO'
            TabOrder = 2
          end
          object UniDBEditCI: TUniDBEdit
            Left = 485
            Top = 28
            Width = 106
            Height = 22
            DataField = 'CI'
            DataSource = DsCadastro
            TabOrder = 6
            Color = clWindow
          end
          object UniLabel5: TUniLabel
            Left = 595
            Top = 9
            Width = 79
            Height = 13
            Caption = 'Data de Entrada'
            TabOrder = 3
          end
          object UniDBDateTimePickerDataEntrada: TUniDBDateTimePicker
            Left = 596
            Top = 28
            Width = 94
            Height = 22
            DateTime = 41780.000000000000000000
            DateFormat = 'dd/MM/yyyy'
            TimeFormat = 'HH:mm:ss'
            TabOrder = 7
            DataField = 'DATA_ENTRADA'
            DataSource = DsCadastro
          end
          object UniDBCheckBoxStatus: TUniDBCheckBox
            Left = 700
            Top = 32
            Width = 61
            Height = 17
            DataField = 'ST'
            DataSource = DsCadastro
            ValueChecked = 'A'
            ValueUnchecked = 'I'
            Caption = 'ATIVO'
            TabOrder = 8
          end
          object UniDBCheckBox2: TUniDBCheckBox
            Left = 767
            Top = 32
            Width = 98
            Height = 17
            DataField = 'EM_TRANSITO'
            DataSource = DsCadastro
            ValueChecked = 'A'
            ValueUnchecked = 'I'
            Caption = 'EM TRANSITO'
            TabOrder = 9
          end
        end
      end
    end
  end
  inherited StatusBar1: TUniStatusBar
    Top = 578
    Width = 983
    ExplicitTop = 578
    ExplicitWidth = 983
  end
  inherited SqlCadastro: TSQLQuery
    SQL.Strings = (
      '')
    Left = 112
  end
  inherited DspCadastro: TDataSetProvider
    Left = 140
  end
  inherited CdsCadastro: TClientDataSet
    Left = 168
  end
  inherited DsCadastro: TDataSource
    Left = 196
  end
  object SQLdigital: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'id_interno'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'select * from biometria'
      'where id_interno = :id_interno')
    SQLConnection = Dm.Conexao
    Left = 752
    Top = 408
  end
  object dshistorico_estudo: TDataSource
    DataSet = cdshistorico_estudo
    Left = 992
    Top = 512
  end
  object cdshistorico_estudo: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dsphistorico_estudo'
    Left = 960
    Top = 512
  end
  object dsphistorico_estudo: TDataSetProvider
    DataSet = SQLhistorico_estudo
    Left = 928
    Top = 512
  end
  object SQLhistorico_estudo: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select *'
      'from historico_estudo h'
      'where id_historico_estudo = -1'
      '')
    Left = 896
    Top = 512
  end
  object DsCelaAntes: TDataSource
    DataSet = CdsCelaAntes
    Left = 989
    Top = 448
  end
  object CdsCelaAntes: TClientDataSet
    Aggregates = <>
    IndexFieldNames = 'idsolario'
    MasterFields = 'id_solario'
    MasterSource = DsSolAntes
    PacketRecords = 0
    Params = <>
    ProviderName = 'DspCelaAntes'
    Left = 957
    Top = 448
  end
  object DspCelaAntes: TDataSetProvider
    DataSet = SqlCelaAntes
    Left = 925
    Top = 448
  end
  object SqlCelaAntes: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'id_solario'
        ParamType = ptInput
        Value = -1
      end>
    SQL.Strings = (
      'select * from cela'
      'where idsolario=:id_solario'
      'order by cela')
    Left = 893
    Top = 448
  end
  object Dsdigital: TDataSource
    DataSet = CDSdigital
    Left = 848
    Top = 408
  end
  object CDSdigital: TClientDataSet
    Aggregates = <>
    IndexFieldNames = 'ID_INTERNO'
    MasterFields = 'ID_INTERNO'
    PacketRecords = 0
    Params = <>
    ProviderName = 'DSPdigital'
    Left = 816
    Top = 408
  end
  object DSPdigital: TDataSetProvider
    DataSet = SQLdigital
    Left = 784
    Top = 408
  end
  object DsSolAntes: TDataSource
    DataSet = CdsSolAntes
    Left = 989
    Top = 400
  end
  object CdsSolAntes: TClientDataSet
    Aggregates = <>
    IndexFieldNames = 'idgaleria'
    MasterFields = 'id_galeria'
    MasterSource = DsGalAntes
    PacketRecords = 0
    Params = <>
    ProviderName = 'DspSolario'
    Left = 957
    Top = 400
  end
  object DspSolAntes: TDataSetProvider
    DataSet = SqlSolAntes
    Left = 925
    Top = 400
  end
  object SqlSolAntes: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'id_galeria'
        ParamType = ptInput
        Value = -1
      end>
    SQL.Strings = (
      'select * from solario'
      'where idgaleria=:id_galeria'
      'order by solario')
    SQLConnection = Dm.Conexao
    Left = 893
    Top = 400
  end
  object DsGalAntes: TDataSource
    DataSet = CdsGalAntes
    Left = 621
    Top = 264
  end
  object CdsGalAntes: TClientDataSet
    Aggregates = <>
    IndexFieldNames = 'idpavilhao'
    MasterFields = 'id_pavilhao'
    MasterSource = DsPavAntes
    PacketRecords = 0
    Params = <>
    ProviderName = 'DspGalAntes'
    Left = 589
    Top = 264
  end
  object DspGalAntes: TDataSetProvider
    DataSet = SqlGalAntes
    Left = 557
    Top = 264
  end
  object SqlGalAntes: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'id_pavilhao'
        ParamType = ptInput
        Value = -1
      end>
    SQL.Strings = (
      'select * from galeria'
      'where idpavilhao=:id_pavilhao'
      'order by galeria')
    SQLConnection = Dm.Conexao
    Left = 525
    Top = 264
  end
  object SQLconspadrao: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQLConnection = Dm.Conexao
    Left = 896
    Top = 48
  end
  object Dspconspadrao: TDataSetProvider
    DataSet = SQLconspadrao
    Left = 924
    Top = 48
  end
  object Cdsconspadrao: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'Dspconspadrao'
    Left = 952
    Top = 48
  end
  object Dsconspadrao: TDataSource
    DataSet = Cdsconspadrao
    Left = 980
    Top = 48
  end
  object DsList_Mudanca_Cela: TDataSource
    DataSet = CdsList_Mudanca_Cela
    Left = 965
    Top = 119
  end
  object CdsList_Mudanca_Cela: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspList_Mudanca_Cela'
    Left = 937
    Top = 119
  end
  object DspList_Mudanca_Cela: TDataSetProvider
    DataSet = SqlList_Mudanca_Cela
    Left = 909
    Top = 119
  end
  object SqlList_Mudanca_Cela: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'id_interno'
        ParamType = ptInput
        Value = 0
      end>
    SQL.Strings = (
      'select '
      'M.DATA,'
      'm.CONFIRMADA,'
      'M.MOTIVO_MOVIMENTACAO,'
      ' v.* from vinc_mudanca_cela v'
      'join mudanca_cela m on (m.id_mudanca_cela=v.id_mudanca_cela)'
      'where v.id_interno = :id_interno')
    SQLConnection = Dm.Conexao
    Left = 881
    Top = 119
  end
  object SqlPavAntes: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftUnknown
        Name = 'id_up'
        ParamType = ptUnknown
      end>
    SQL.Strings = (
      'select * from pavilhao'
      'where id_up = :id_up'
      'order by pavilhao')
    SQLConnection = Dm.Conexao
    Left = 885
    Top = 205
  end
  object DspPavAntes: TDataSetProvider
    DataSet = SqlPavAntes
    Left = 917
    Top = 205
  end
  object CdsPavAntes: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspPavAntes'
    Left = 949
    Top = 205
  end
  object DsPavAntes: TDataSource
    DataSet = CdsPavAntes
    Left = 981
    Top = 205
  end
  object DsMudancaCela: TDataSource
    DataSet = CdsMudancaCela
    Left = 972
    Top = 280
  end
  object CdsMudancaCela: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspMudancaCela'
    Left = 944
    Top = 280
  end
  object DspMudancaCela: TDataSetProvider
    DataSet = SqlMudancaCela
    Left = 916
    Top = 280
  end
  object SqlMudancaCela: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'ID_MUDANCA_CELA'
        ParamType = ptInput
        Value = 0
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM MUDANCA_CELA'
      'WHERE ID_MUDANCA_CELA= :ID_MUDANCA_CELA')
    SQLConnection = Dm.Conexao
    Left = 888
    Top = 280
  end
  object DsPavilhao: TDataSource
    DataSet = CdsPavilhao
    Left = 832
    Top = 133
  end
  object CdsPavilhao: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspPavilhao'
    Left = 800
    Top = 133
  end
  object DspPavilhao: TDataSetProvider
    DataSet = SqlPavilhao
    Left = 768
    Top = 133
  end
  object SqlPavilhao: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select * from pavilhao order by pavilhao')
    SQLConnection = Dm.Conexao
    Left = 736
    Top = 133
  end
  object DsCela: TDataSource
    DataSet = CdsCela
    Left = 832
    Top = 272
  end
  object CdsCela: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspCela'
    Left = 800
    Top = 272
  end
  object DspCela: TDataSetProvider
    DataSet = SqlCela
    Left = 768
    Top = 272
  end
  object SqlCela: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'id_solario'
        ParamType = ptInput
        Value = -1
      end>
    SQL.Strings = (
      'select * from cela'
      'where idsolario=:id_solario'
      'order by cela')
    SQLConnection = Dm.Conexao
    Left = 736
    Top = 272
  end
  object DsSolario: TDataSource
    DataSet = CdsSolario
    Left = 832
    Top = 224
  end
  object CdsSolario: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspSolario'
    Left = 800
    Top = 224
  end
  object DspSolario: TDataSetProvider
    DataSet = SqlSolario
    Left = 768
    Top = 224
  end
  object SqlSolario: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'id_galeria'
        ParamType = ptInput
        Value = -1
      end>
    SQL.Strings = (
      'select * from solario'
      'where idgaleria=:id_galeria'
      'order by solario')
    SQLConnection = Dm.Conexao
    Left = 736
    Top = 224
  end
  object DsGaleria: TDataSource
    DataSet = CdsGaleria
    Left = 832
    Top = 176
  end
  object CdsGaleria: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspGaleria'
    Left = 800
    Top = 176
  end
  object DspGaleria: TDataSetProvider
    DataSet = SqlGaleria
    Left = 768
    Top = 176
  end
  object SqlGaleria: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'id_pavilhao'
        ParamType = ptInput
        Value = -1
      end>
    SQL.Strings = (
      'select * from galeria'
      'where idpavilhao=:id_pavilhao'
      'order by galeria')
    SQLConnection = Dm.Conexao
    Left = 736
    Top = 176
  end
  object DSHISTORICO_interno: TDataSource
    DataSet = CDSHISTORICO_interno
    Left = 828
    Top = 96
  end
  object CDSHISTORICO_interno: TClientDataSet
    Aggregates = <>
    IndexFieldNames = 'IDINTERNO'
    MasterFields = 'ID_INTERNO'
    PacketRecords = 0
    Params = <>
    ProviderName = 'DSPHISTORICO_interno'
    Left = 800
    Top = 96
    object CDSHISTORICO_internoIDHISTORICO_INTERNO: TIntegerField
      FieldName = 'IDHISTORICO_INTERNO'
      Required = True
    end
    object CDSHISTORICO_internoDESCRICAO: TStringField
      DisplayLabel = 'DESCRI'#199#195'O'
      FieldName = 'DESCRICAO'
      Required = True
      Size = 16386
    end
    object CDSHISTORICO_internoDATA_HORA: TSQLTimeStampField
      DisplayLabel = 'DATA'
      FieldName = 'DATA_HORA'
      Required = True
    end
    object CDSHISTORICO_internoIDINTERNO: TIntegerField
      FieldName = 'IDINTERNO'
      Required = True
    end
    object CDSHISTORICO_internoSTATUS: TStringField
      FieldName = 'STATUS'
      Size = 1
    end
    object CDSHISTORICO_internoMOTIVO_SAIDA: TStringField
      FieldName = 'MOTIVO_SAIDA'
      Size = 50
    end
    object CDSHISTORICO_internoIDPROCEDENCIA: TIntegerField
      FieldName = 'IDPROCEDENCIA'
    end
    object CDSHISTORICO_internoIDUP: TIntegerField
      FieldName = 'IDUP'
    end
  end
  object DSPHISTORICO_interno: TDataSetProvider
    DataSet = SQLHISTORICO_interno
    Left = 772
    Top = 96
  end
  object SQLHISTORICO_interno: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'id_interno'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'select *'
      'from historico_interno'
      'where idinterno=:id_interno'
      'order by data_hora desc')
    SQLConnection = Dm.Conexao
    Left = 744
    Top = 96
  end
  object DsMovimentacaoExterna: TDataSource
    DataSet = CdsMovimentacaoExterna
    Left = 836
    Top = 8
  end
  object CdsMovimentacaoExterna: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspMovimentacaoExterna'
    Left = 808
    Top = 8
  end
  object DspMovimentacaoExterna: TDataSetProvider
    DataSet = SqlMovimentacaoExterna
    Left = 716
    Top = 80
  end
  object SqlMovimentacaoExterna: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'ID_INTERNO'
        ParamType = ptInput
        Value = 0
      end>
    SQL.Strings = (
      'SELECT '
      '    T.DATA as "Dt movimenta'#231#227'o",'
      '    U.nome_up as "Origem",'
      
        '    IIF(T.TIPO_DESTINO<>'#39'U'#39', '#39'LOCAL: '#39'||D.DESTINO, U2.NOME_UP) a' +
        's "Destino",'
      '    T.TIPO_DOCUMENTO as "Tipo Doc",'
      '    T.NUMERO_DOCUMENTO as "N. Doc",'
      '    T.MOTIVO_MOVIMENTACAO as "Motivo da movimenta'#231#227'o"'
      'FROM TRANSFERENCIA_INTERNO T'
      
        '   INNER JOIN vinc_transferencia_interno V ON (V.id_transferenci' +
        'a_interno=T.id_transferencia_interno)'
      '   INNER JOIN INTERNO I ON (V.id_interno=I.id_interno)'
      '   LEFT JOIN DESTINO D ON (D.ID_DESTINO = T.ID_DESTINO)'
      '   LEFT JOIN UNIDADE_PENAL U ON (T.ID_ORIGEM = U.ID_UP)'
      '   LEFT JOIN UNIDADE_PENAL U2 ON (T.ID_UP_DESTINO = U2.ID_UP)'
      'where v.ID_INTERNO = :ID_INTERNO'
      'ORDER BY T.DATA DESC')
    SQLConnection = Dm.Conexao
    Left = 688
    Top = 80
  end
  object DspMovimento: TDataSetProvider
    DataSet = SqlMovimento
    Left = 644
    Top = 136
  end
  object SqlMovimento: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select id_mov_interno, id_interno, id_unidade_penal,'
      'tipo_movimentacao, data_movimentacao, hora_movimentacao,'
      'id_funcionario,'
      
        '(SELECT proedencia from procedencia WHERE procedencia.id_procede' +
        'ncia=mov_interno.id_procedencia) AS id_procedencia,'
      
        '(SELECT DESTINO from destino WHERE destino.id_destino=mov_intern' +
        'o.id_destino) AS  id_destino'
      'from mov_interno'
      'WHERE IDPAVILHAO IS NOT NULL')
    SQLConnection = Dm.Conexao
    Left = 616
    Top = 136
  end
  object dshistorico_trabalho: TDataSource
    DataSet = cdshistorico_trabalho
    Left = 576
    Top = 216
  end
  object cdshistorico_trabalho: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dsphistorico_trabalho'
    Left = 544
    Top = 216
  end
  object dsphistorico_trabalho: TDataSetProvider
    DataSet = SQLhistorico_trabalho
    Left = 512
    Top = 216
  end
  object SQLhistorico_trabalho: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select * from historico_trabalho'
      'where id_historico_trabalho = -1')
    SQLConnection = Dm.Conexao
    Left = 480
    Top = 216
  end
  object SqlSelectInterno: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select '
      '    interno.nome_interno,'
      '    unidade_penal.sigla,'
      '    unidade_penal.nome_up,'
      '    pavilhao.pavilhao,'
      '    solario.solario,'
      '    cela.cela,'
      '    interno.id_interno,'
      '    interno.id_up,'
      'interno.idpavilhao,'
      'interno.idgaleria,'
      'interno.idsolario,'
      'interno.st,'
      'iif(interno.st='#39'A'#39','#39'ATIVO'#39', '#39'INATIVO'#39') status,'
      'interno.id_me'
      'from interno'
      '  left join cela on (cela.id_cela = interno.idcela)'
      
        '  left join pavilhao on (interno.idpavilhao = pavilhao.id_pavilh' +
        'ao)'
      
        '  left join unidade_penal on (pavilhao.id_up = unidade_penal.id_' +
        'up)'
      '  left join solario on (interno.idsolario = solario.id_solario)'
      'where interno.nome_interno is not null')
    SQLConnection = Dm.Conexao
    Left = 16
    Top = 4
  end
  object DsDocumentos: TDataSource
    DataSet = CdsDocumentos
    Left = 576
    Top = 160
  end
  object CdsDocumentos: TClientDataSet
    Aggregates = <>
    IndexFieldNames = 'ID_INTERNO'
    MasterFields = 'ID_INTERNO'
    PacketRecords = 0
    Params = <>
    ProviderName = 'DspDocumentos'
    Left = 544
    Top = 160
  end
  object DspDocumentos: TDataSetProvider
    DataSet = SqlDocumentos
    Left = 512
    Top = 160
  end
  object SqlDocumentos: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'ID_INTERNO'
        ParamType = ptInput
        Value = -1
      end>
    SQL.Strings = (
      'select * from DOCUMENTOS_INCLUSAO'
      'where ID_INTERNO = :ID_INTERNO'
      '')
    SQLConnection = Dm.Conexao
    Left = 480
    Top = 160
  end
  object Sqlvinc_transferencia_interno: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'id_transferencia_interno'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'select * from vinc_transferencia_interno'
      'where id_transferencia_interno = :id_transferencia_interno')
    SQLConnection = Dm.Conexao
    Left = 297
    Top = 175
  end
  object Dspvinc_transferencia_interno: TDataSetProvider
    DataSet = Sqlvinc_transferencia_interno
    Left = 325
    Top = 175
  end
  object Cdsvinc_transferencia_interno: TClientDataSet
    Aggregates = <>
    IndexFieldNames = 'ID_TRANSFERENCIA_INTERNO'
    MasterFields = 'ID_TRANSFERENCIA_INTERNO'
    MasterSource = DsTransferenciaInterno
    PacketRecords = 0
    Params = <>
    ProviderName = 'Dspvinc_transferencia_interno'
    Left = 353
    Top = 175
  end
  object Dsvinc_transferencia_interno: TDataSource
    DataSet = Cdsvinc_transferencia_interno
    Left = 382
    Top = 175
  end
  object DsTransferenciaInterno: TDataSource
    DataSet = CdsTransferenciaInterno
    Left = 380
    Top = 144
  end
  object CdsTransferenciaInterno: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspTransferenciaInterno'
    Left = 352
    Top = 144
  end
  object DspTransferenciaInterno: TDataSetProvider
    DataSet = SqlTransferenciaInterno
    Left = 324
    Top = 144
  end
  object SqlTransferenciaInterno: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'id_transferencia_interno'
        ParamType = ptInput
        Value = -1
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM transferencia_interno'
      'where id_transferencia_interno= :id_transferencia_interno')
    SQLConnection = Dm.Conexao
    Left = 296
    Top = 144
  end
  object SqlSelectInternoInativo: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select '
      '   nome_interno,'
      '   '#39'INAT'#39' sigla,'
      '   '#39#39' pavilhao,'
      '   '#39#39' solario,'
      '   '#39#39' cela,'
      '    id_interno,'
      '    idpavilhao,'
      '    idgaleria,'
      '    idsolario,'
      '    numero_roupa,'
      ' '#39'INATIVO'#39' status,'
      '    interno.id_me'
      'from interno')
    SQLConnection = Dm.Conexao
    Left = 184
    Top = 140
  end
  object SqlConsultaTodos: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select first 50'
      '    interno.nome_interno,'
      '    interno.rgi,'
      '    '#39#39' as  nome_interno_soundex,'
      '    '#39#39' as sigla,'
      '    '#39#39' as pavilhao,'
      '    '#39#39' as galeria,'
      '    '#39#39' as solario,'
      '    '#39#39' as cela,'
      '    interno.id_interno,'
      '    interno.id_up,'
      '    interno.idpavilhao,'
      '    interno.idgaleria,'
      '    interno.idsolario,'
      '    interno.idcela,'
      '    coalesce(interno.st,'#39'I'#39') as st,'
      '    interno.numero_roupa,'
      '    iif(coalesce(interno.st,'#39'I'#39')='#39'A'#39','#39'ATIVO'#39', '#39'INATIVO'#39') status,'
      '    interno.em_transito,'
      '    interno.id_me'
      'from interno'
      'where coalesce(interno.st,'#39'I'#39') <> '#39'A'#39
      '')
    SQLConnection = Dm.Conexao
    Left = 208
    Top = 112
  end
  object SqlSoundex: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select '
      '    interno.nome_interno,'
      
        '    (SELECT RET FROM SOUNDEX(INTERNO.NOME_INTERNO)) nome_interno' +
        '_soundex,'
      '    unidade_penal.sigla,'
      '    unidade_penal.nome_up,'
      '    pavilhao.pavilhao,'
      '    solario.solario,'
      '    cela.cela,'
      '    interno.id_interno,'
      '    interno.id_up,'
      '    interno.idpavilhao,'
      '    interno.idgaleria,'
      '    interno.idsolario,'
      '    interno.st,'
      '    iif(interno.st='#39'A'#39','#39'ATIVO'#39', '#39'INATIVO'#39') status,'
      '    interno.em_transito,'
      '    interno.id_me'
      'from interno'
      '   join cela on (cela.id_cela = interno.idcela)'
      '   join pavilhao on (interno.idpavilhao = pavilhao.id_pavilhao)'
      '   join unidade_penal on (pavilhao.id_up = unidade_penal.id_up)'
      '   join solario on (interno.idsolario = solario.id_solario)'
      'where interno.nome_interno is not null'
      'order by interno.nome_interno')
    SQLConnection = Dm.Conexao
    Left = 248
    Top = 88
  end
  object DsConsulta: TDataSource
    DataSet = CdsConsulta
    Left = 332
    Top = 48
  end
  object CdsConsulta: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspConsulta'
    Left = 304
    Top = 48
  end
  object DspConsulta: TDataSetProvider
    DataSet = SqlConsulta
    Left = 276
    Top = 48
  end
  object SqlConsulta: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQLConnection = Dm.Conexao
    Left = 248
    Top = 48
  end
  object SqlConsultaBackup: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select '
      'interno.nome_interno,'
      'interno.rgi,'
      '    '#39#39' as  nome_interno_soundex,'
      '    unidade_penal.sigla,'
      '    unidade_penal.nome_up,'
      '    pavilhao.pavilhao,'
      '    galeria.galeria,'
      '    solario.solario,'
      '    cela.cela,'
      '    interno.id_interno,'
      '    interno.id_up,'
      '    interno.idpavilhao,'
      '    interno.idgaleria,'
      '    interno.idsolario,'
      '    interno.idcela,'
      '    coalesce(interno.st,'#39'I'#39') as st,'
      '    interno.numero_roupa,'
      '    iif(coalesce(interno.st,'#39'I'#39')='#39'A'#39','#39'ATIVO'#39', '#39'INATIVO'#39') status,'
      '    interno.em_transito,'
      '    interno.id_me'
      'from interno'
      '  left join cela on (cela.id_cela = interno.idcela)'
      
        '  left join pavilhao on (interno.idpavilhao = pavilhao.id_pavilh' +
        'ao)'
      
        '  left join unidade_penal on (pavilhao.id_up = unidade_penal.id_' +
        'up)'
      '  left join galeria on (interno.idgaleria = galeria.id_galeria)'
      '  left join solario on (interno.idsolario = solario.id_solario)'
      'where coalesce(interno.nome_interno,'#39#39') <> '#39#39
      '')
    SQLConnection = Dm.Conexao
    Left = 216
    Top = 48
  end
  object DsVinc_Mudanca_Cela: TDataSource
    DataSet = CdsVinc_Mudanca_Cela
    Left = 445
    Top = 263
  end
  object CdsVinc_Mudanca_Cela: TClientDataSet
    Aggregates = <>
    IndexFieldNames = 'ID_MUDANCA_CELA'
    MasterFields = 'ID_MUDANCA_CELA'
    MasterSource = DsMudancaCela
    PacketRecords = 0
    Params = <>
    ProviderName = 'DspVinc_Mudanca_Cela'
    Left = 417
    Top = 263
  end
  object DspVinc_Mudanca_Cela: TDataSetProvider
    DataSet = SqlVinc_Mudanca_Cela
    Left = 389
    Top = 263
  end
  object SqlVinc_Mudanca_Cela: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'id_mudanca_cela'
        ParamType = ptInput
        Value = 0
      end>
    SQL.Strings = (
      'select * from vinc_mudanca_cela'
      'where id_mudanca_cela = :id_mudanca_cela'
      'order by id_mudanca_cela')
    SQLConnection = Dm.Conexao
    Left = 361
    Top = 263
  end
  object UniFileUploadFoto: TUniFileUpload
    MaxAllowedSize = 10485760
    Filter = '*.jpg'
    Title = 'Enviar Foto'
    Messages.Uploading = 'Enviando...'
    Messages.PleaseWait = 'Por favor confirme'
    Messages.Cancel = 'Cancelar'
    Messages.Processing = 'Processando...'
    Messages.UploadError = 'Erro ao Enviar...'
    Messages.Upload = 'Enviar'
    Messages.NoFileError = 'Por favor selecione o arquivo'
    Messages.BrowseText = 'Procurar'
    Left = 64
    Top = 192
  end
end
